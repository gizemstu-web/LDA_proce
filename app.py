...
def main():
    ...
    st.markdown("""
    Bu uygulama, İtalyanca edebi metinler ve seyahatnameler üzerinde **Latent Dirichlet Allocation** algoritmasını kullanarak gizli temaları keşfetmenizi sağlar.
    """)

    # --- SIDEBAR: PARAMETRE AYARLARI ---
    st.sidebar.header("Model Parametreleri")
    
    uploaded_file = st.sidebar.file_uploader("Veri Setini Yükle (CSV veya TXT)", type=['csv', 'txt'])
    ...
    alpha = st.sidebar.selectbox("Alpha (Belge-Konu Yoğunluğu)", ["symmetric", "asymmetric", "auto"])
